...
// No se lee ninguna preferencia antigua del navegador.
function getBase() { return config.PUBLIC_BASE_URL; }
...
async function init(){
 if(!window.supabase||!window.qrcode){const el=isRedirect?$('redirect-screen'):$('auth-screen');el.hidden=false;const text='No se pudieron cargar las dependencias. Revisa tu conexión y recarga la página.';if(isRedirect){$('redirect-title').textContent='No se pudo iniciar';$('redirect-message').textContent=text;}else{$('auth-message').textContent=text;setAuthBusy(true);}return;}
 ...
 $('signup-form').addEventListener('submit', async e => {
  ...
  setAuthBusy(true);
  try {
   const {data,error}=await db.auth.signUp({email,password,options:{emailRedirectTo:validBase(getBase())}});
   if(error)throw error;
   $('signup-password').value=''; $('signup-confirm').value=''; $('email').value=email;
   if(data.session){await showSession(data.session);toast('Tu cuenta está lista. Bienvenido a QR Studio.');}
   else {setAuthMode(false);authMessage('Revisa tu correo para confirmar el registro y después inicia sesión. Si ya tienes una cuenta, entra con tu contraseña habitual.',true);}
  } catch(err) {authMessage(message(err));} finally {setAuthBusy(false);}
 });
 ...
 const{data,error}=await db.auth.getSession();if(error)authMessage(message(error));else await showSession(data.session);
 if(callbackError&&!data?.session)authMessage('El enlace de confirmación no es válido o ha caducado. Intenta iniciar sesión o solicita ayuda al administrador.');
}
