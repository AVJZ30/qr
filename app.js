...
const storageKey = 'avj-qr-public-url:' + config.SUPABASE_URL;
...
function validBase(value) {
 const u = new URL(validURL(value));
 if (u.protocol !== 'https:') throw new Error('La dirección pública debe comenzar con https://.');
 if (u.search || u.hash) throw new Error('La dirección de tu web no debe contener parámetros ni #.');
 if (u.hostname === new URL(config.SUPABASE_URL).hostname) throw new Error('Usa la dirección de tu página, no la de Supabase.');
 if (/localhost|^127\.|^\[::1\]$/.test(u.hostname)) throw new Error('Usa un dominio público accesible desde otro dispositivo.');
 return u.href;
}
function getBase() { try { return localStorage.getItem(storageKey) || config.PUBLIC_BASE_URL || ''; } catch { return config.PUBLIC_BASE_URL || ''; } }
...
function openSettings() {$('public-url').value=getBase();$('settings-error').textContent='';$('settings').showModal();}
function openEditor(row=null) {
 if(!row){try{validBase(getBase());}catch{openSettings();toast('Primero guarda la dirección pública de tu web.');return;}}
...
async function showSession(session){
 user=session?.user||null;$('auth-screen').hidden=!!user;$('dashboard').hidden=!user;
...
async function init(){
 if(!window.supabase||!window.qrcode){const el=isRedirect?$('redirect-screen'):$('auth-screen');el.hidden=false;const text='No se pudieron cargar las dependencias. Revisa tu conexión y recarga la página.';if(isRedirect){$('redirect-title').textContent='No se pudo iniciar';$('redirect-message').textContent=text;}else{$('auth-message').textContent=text;$('login-form').querySelector('button').disabled=true;}return;}
 db=window.supabase.createClient(config.SUPABASE_URL,config.SUPABASE_PUBLISHABLE_KEY,{auth:{persistSession:!isRedirect,autoRefreshToken:!isRedirect,detectSessionInUrl:false},global:{fetch:(url,options)=>fetch(url,{...options,cache:'no-store'})}});
 ...
 $('login-form').addEventListener('submit',async e=>{e.preventDefault();const b=e.submitter;b.disabled=true;$('auth-message').textContent='';try{const{error}=await db.auth.signInWithPassword({email:$('email').value.trim(),password:$('password').value});if(error)throw error;$('password').value='';}catch(err){$('auth-message').textContent=message(err);}finally{b.disabled=false;}});
 $('logout').onclick=async()=>{const{error}=await db.auth.signOut({scope:'local'});if(error)toast(message(error));else await showSession(null);};
 $('new-qr').onclick=()=>openEditor();$('empty-create').onclick=()=>openEditor();$('open-settings').onclick=openSettings;$('refresh').onclick=loadRows;
 ...
 $('settings-form').onsubmit=e=>{e.preventDefault();try{const base=validBase($('public-url').value);localStorage.setItem(storageKey,base);$('settings').close();toast('Dirección guardada para los nuevos QR.');}catch(err){$('settings-error').textContent=message(err);}};
 ...
 else{const token=crypto.randomUUID().replaceAll('-','');const permanent=new URL(validBase(getBase()));permanent.searchParams.set('q',token);qrFor(permanent.href);result=await db.from('qr_links').insert({...payload,owner_id:user.id,token,public_url:permanent.href}).select().single();}
 ...
 db.auth.onAuthStateChange((event,session)=>{if(event==='SIGNED_IN'||event==='SIGNED_OUT')setTimeout(()=>showSession(session),0);});
 const{data,error}=await db.auth.getSession();if(error)$('auth-message').textContent=message(error);else await showSession(data.session);
}
